...
| **"I'm locked into one model vendor."** | One `Provider` seam, three adapters: **Anthropic, OpenAI, OpenRouter.** Model selection is `role → provider:model`. The cheap executor escalates to a strong advisor on demand. And one `CodingBackend` seam, three backends: the **native loop, Codex, Claude Code** — configure several (`-backends native,codex,claude-code`) and the **Trust Ledger** routes to the one that historically *wins verifier-judged races on your codebase*, racing them on a hard task and letting the verifier pick the winner (`nilcore trust` shows the scoreboard). |

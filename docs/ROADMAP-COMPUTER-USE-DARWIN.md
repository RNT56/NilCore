...
| CU-MAC-T07 | 4 | Permission/onboarding probe — live TCC check + posted-but-no-effect detection | T05 | `…/permissions.go` | ✅ **BUILT** — `--probe` / `nilcore desktop --mac-probe` / `make desktop-mac-probe`; live `screencapture` attempt (not cached `AXIsProcessTrusted`); cliclick-presence + actionable "grant …, then restart" guidance; exits non-zero when not ready |
...
| CU-MAC-T09 | 5 | Host-control transport + the darwin/host gate (`NILCORE_DESKTOP_HOST`, forced approval, per-app allowlist, kill-switch) | T06,T07 | `internal/desktopsession/hosttransport.go`, `cmd/nilcore/desktop.go`, `…/hardening.go` | ✅ **BUILT** — host transport + `LaunchHost` + `--mac-host` gate (`NILCORE_DESKTOP_HOST=1` + forced approval) **plus** the kill-switch (`$NILCORE_DESKTOP_STOP`) + per-app allowlist (`NILCORE_DESKTOP_ALLOW_APPS`), both fail-closed before every mutating act (hermetically tested) |

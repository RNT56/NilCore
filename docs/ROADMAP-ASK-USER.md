...
> **Historical (pre-`ask_user`).** This section is the contradiction that motivated the work; it has since
> been RESOLVED (`ask_user` SHIPPED — §6b). The "What the code actually did" column below describes the
> state *before* this roadmap was built, not current behaviour.
...
Before this work, that was **designed but entirely unwired**, and the code did the opposite:

| What PERSONA §2 promises | What the code did before `ask_user` | Site |
|---|---|---|
...
Two facts uncovered during design shaped the work and were easy to get wrong (both now RESOLVED — noted for the record):

1. **`AwaitingGate` was dormant at design time.** `session.Phase` `AwaitingGate` (now `internal/session/state.go:43`) was **declared but never assigned** — the real gate blocked *inside* `agent.Orchestrator`/`ConsoleApprover` one layer below the session, invisible to the `Session.Phase` machine. So "make `AwaitingInput` mirror `AwaitingGate`" would have copied a precedent that did not yet exist; the parked-phase machinery was genuinely new. **This is fixed:** `AwaitingInput` shipped (assigned at `internal/session/ask.go:66`) and `AU-T05b` later made `AwaitingGate` real too — it is now assigned when a session-backed drive parks on the human gate (`internal/session/gate.go:72`).
2. **The chat front door had a latent stdin race.** `ConsoleApprover` read `os.Stdin` directly (`internal/policy/approver.go`) while the REPL reader scanned the *same* `os.Stdin` — competing `bufio` readers. `ask_user` forced unifying input (§4). **This is fixed:** interactive line-REPL drives now route the gate through the session `AwaitingGate` approver via `gateApproverFor` (`cmd/nilcore/chat.go:1439`) instead of a stdin-racing `ConsoleApprover`; the fallback `ConsoleApprover` is used only when neither is wired (non-interactive/test build).
...
- **AU-T06b — SHIPPED (#70).** Native inline buttons / checkboxes for choices on Telegram (`inline_keyboard`) + Slack (Block Kit) via the optional `channel.ChoicePoster` capability (`internal/channel/choices.go` + the per-transport `choices.go`), so the frozen `channel.go` contract is untouched. A button tap is turned into an ordinary authorized `TaskRequest` (Sender = clicker) flowing the SAME path a typed answer does (I7); the transport formats only INDICES into the answer grammar. A free-text reply remains the fallback. Tested per transport (single/multi-select, Done, unauthorized-tap-dropped, ChoicePoster routing).

- **AU-T05b — SHIPPED.** The §4 line-REPL/`AwaitingGate` unification is now complete. The native + TUI gate/ask serialization landed in #70; the remaining gap — a chat turn that *escalates* to a **supervise/project** drive (those drives both ask the human AND hit the human promote gate) — is now closed: the session-backed gate approver (`DriveInput.Gate`) is threaded through `RunSuperviseFunc`/`RunProjectFunc` into the supervise/project drivers, and the chat closures select it via `gateApproverFor`, so a line-REPL escalated drive parks `AwaitingGate` (single REPL reader) instead of spawning a second `ConsoleApprover` that races stdin. Serve is unchanged (its channel approver already routes the gate over the transport). Safety: ask and gate are never simultaneous in a supervise drive (single-owner at a round/integrate boundary), so `AwaitingInput`/`AwaitingGate` stay mutually exclusive — no new concurrent-park machinery. (This invariant is load-bearing; a future `AU-T08` `scope=let_run` must revisit it.)

- **AU-T08 — partially shipped; the rest deferred (infeasible now).** The supervisor `ask_user` **seam + tool** (`AU-T08a`) shipped in #70 (the multi-agent root planner gains a nil-able `AskUser`). The full **§0.1 concurrency scope** (`scope: let_run|pause|stop` resolved by the supervisor against the Kahn-wave pool + peer bus) remains **unbuilt** — an architecture review found it needs pause/resume/stop machinery across the scheduler, dispatch, and peer bus that does not safely exist yet (high risk), so it stays deferred until that machinery is built.

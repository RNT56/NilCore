...
| Phase-2 channel return | **flattened `[]string`** (one resolved string per question) | keeps the frozen `channel.go` surface minimal; `[]Question` on input |
...
**Deferred (as designed):** the §4 line-REPL/`AwaitingGate` unification was **not needed for Phase 1** — the interactive chat native drive does not invoke the console approver (its `run` tool is `CommandGuard`-gated, and `Orchestrator.Gate` is reached only from project/selfimprove/trigger/mcp), so no gate can race the REPL reader during a native ask. It remains a prerequisite for wiring `ask_user` into a drive that *can* gate (a chat `/execute` that escalates to supervise/project), tracked with **AU-T05b**. **Phase 2** (`AU-T06/T07`, the `channel.go` contract extension for Telegram/Slack) and **`AU-T08`** (the §0.1 concurrency scope) are unbuilt.

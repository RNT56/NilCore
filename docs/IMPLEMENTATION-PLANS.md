...
- **Verify:** the `browser-e2e` CI job (like the `sandbox-linux` job; `.github/workflows/ci.yml`) drives `nilcore-browser` against a fixture page on the runner's preinstalled Chrome and asserts the JSON observation (title + a real screenshot + extracted text) AND a non-zero exit when no browser is present (**fail-closed**). Hermetic unit tests (`make verify`) cover the JSON encoder, the arg builder, the DOM→title extractor, and the fail-closed path.

...
- **Wayland / macOS unsupported** — the design is Linux-X11 (`xdotool`/`scrot`); verifiable only in-container (there is no `desktop-e2e` CI job — run `make desktop-e2e` locally), never directly on a display-less macOS dev host (CU-T01/T05/T09).

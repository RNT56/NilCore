...
> _Shipped note:_ the semantic index shipped, but **not** on `sqlite-vec` (cgo — rejected under I6). The vector path is the pure-Go, content-hash-cached HNSW of the deferred **D2** item (`internal/codeintel/semantic/hnsw.go`), activated opt-in via `NILCORE_EMBED_KEY` (see P10-T03/P10-T04); absent a key it degrades to lexical+graph.

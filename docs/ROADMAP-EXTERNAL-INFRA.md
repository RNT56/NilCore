...
**Why it requires external infra.** Everything about NilCore's parallelism is in-process on one host (see §1) — including **verified swarm mode** (Phase 12, `docs/SWARM.md`): `nilcore swarm --agents 300 --concurrency 40` is the **single-host projection** of a fleet, a bounded in-process pool over `scheduler`/`spawn.DAGScheduler` with local-restart resume and no standing authority. The line it does **not** cross — multi-host shard dispatch, cross-host task-state, a remote control plane — is exactly this EXT-01 boundary. A fleet is a *categorical* jump: a scheduler/control plane that leases tasks to remote workers, provisions and reclaims VMs, persists task state across hosts, and exposes a dashboard. Today durable resume re-drives work on a **single host**: a native single task and — since the multi-agent `RunState`/`ResumePlan` machinery was wired — an in-flight supervise/project run, both replayed on process restart under a deny-default approver (`cmd/nilcore/resume.go` `resumeSupervise`, reached at serve boot; `internal/agent/durability.go`). What stays uncrossed is **cross-host** task-state: no control plane persists shard state across hosts to re-lease an orphaned shard to a new VM.
...
- Reuse the seams, don't rebuild the loop: the unit of remote work is still a `backend.CodingBackend.Run` (`I1`), wrapped — the role/worker abstraction is already "configuration over the one loop" (`internal/roster/roster.go:1-7`). `model.Resilient` already accepts an **ordered** `[]Provider` for failover but is passed a single element today (`cmd/nilcore/main.go:1992,2045`) — a fleet would light up multi-provider routing here.
- Replace the in-process `scheduler` with a leasing control plane that submits the same `scheduler.Task{ID, Run}` unit (`internal/scheduler/scheduler.go:29-32`) to remote workers; preserve the error-isolation + ctx-cancellation contract.
- Extend the multi-agent `RunState`/`ResumePlan` machinery (`internal/agent/durability.go`) — today wired for **single-host** supervise/project resume (`resumeSupervise`) — to cross-host handoff/leasing, keeping `Checkpoint`'s store-backed model.
...
- Deploy/land stays a gated `policy.GateAction` through the human approver; nil approver default-denies (`internal/policy/gateaction.go:114-123`).

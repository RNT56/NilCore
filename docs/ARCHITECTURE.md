...
**agentic-flows consumer seam (`internal/agenticflows` + `nilcore flows`; `docs/AGENTIC-FLOWS.md`, additive, opt-in).** NilCore is the **sandboxed-worker consumer** of the portable [`agentic-flows`](https://github.com/RNT56/agentic-flows) workflow contract shared across the RNT56 projects. `internal/agenticflows` is a pure leaf (imports `spawn`/`sandbox`/`summarize`, never the orchestrator) that maps a DECODED flow onto NilCore's seams — `agent_task` nodes → `spawn.Subtask` worker dispatch, `tool` nodes → sandbox plans — and reports the capabilities NilCore does/doesn't advertise. `nilcore flows validate` is a no-execution preflight gate (supported core + capability check, exit-1 on unconsumable); `nilcore flows run` composes the `agent_task` nodes into a goal and runs them through the proven `decompose` preset (each an independent verified sub-run integrated into one re-verified tip — I2). Flows are consumed as **JSON** (stdlib-only — I6; no YAML module), inert data (I7), with edges derived from the flow's `produces`→`requires` dataflow. The contract stays external (NilCore vendors no flow code, adds no module); the decoder ignores unknown fields (forward-compatible with the evolving spec).

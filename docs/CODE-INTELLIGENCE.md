...
A pure-Go regexp scan (the `search` tool — `regexp.Compile` + `filepath.WalkDir` in `internal/tools/fs.go`, not an external `ripgrep`) plus exact symbol lookup. The cheapest, most precise lens: known identifiers, error strings, literal scans. The walk skips `.git` and never follows a symlink (I4 worktree confinement); it does **not** prune dependency trees — that pruning (`node_modules`, `vendor`, `__pycache__`, `dist`, `build`, hidden dirs) belongs to the codeintel *index* walk (`sourceFilesUnder`, `internal/tools/codeintel.go`), not to the `search` tool, which scans them.
...
Pure-Go multi-language parser seam (Go via `go/parser` — the one precise backend — plus broad heuristic line-scanner backends for Python, TypeScript/JavaScript, Rust, Java, C, C++, C#, Ruby, PHP, Kotlin, Swift, Scala, Dart, Zig, Bash, Lua, Elixir, and SQL — **19 languages / 34 extensions**: `.go .py .js .jsx .ts .tsx .mjs .cjs .rs .java .c .h .cc .cpp .cxx .hpp .hh .hxx .cs .rb .php .kt .kts .swift .scala .sc .dart .zig .sh .bash .lua .ex .exs .sql`; CGO-free, not tree-sitter; `ast.SupportedExtensions` drives the walks) · LSP clients (the precise lens for type resolution via `NILCORE_LSP_COMMAND`, optional) · SQLite for the graph (immediate edges in SQL; transitive reachability via a Go-side BFS, not recursive CTEs) · a content-hash-cached pure-Go HNSW vector index for the semantic lens (`internal/codeintel/semantic/hnsw.go`) · code-aware embeddings via an OpenAI-compatible embedder (`internal/embed`, opt-in via `NILCORE_EMBED_KEY` / `NILCORE_EMBED_MODEL`) · the pure-Go `search` tool (lexical regexp scan, and the fallback when embeddings are off) · file-watching (incremental updates) · SCIP alignment for the semantic layer.
...
| P3-T10 | `graph/` | code graph in SQLite + structural queries (immediate edges in SQL; Go-side BFS for transitive reachability) |

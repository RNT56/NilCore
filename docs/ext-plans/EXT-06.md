...
- A captured lease therefore yields at most: a narrow secret, already expiring, whose only push path runs through a proxy that refuses anything but the working branch — and base-branch land still requires the human gate (`gateaction.go:86-94`). **No single compromise reaches the fleet master credential or the base branch.**

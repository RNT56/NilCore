// Package loopctl is the shared cancel-cause discriminator for the
// conversational front door (C1-T02). When a bounded model loop (native or
// supervisor) wraps a single iteration's model call in a cancellable context, a
// returned cancellation can mean one of three very different things, and the
// loop must react differently to each:
//
...
//     deliberately cancelled its in-flight model call to fold the steering text
//     in at the next boundary. This is NOT an error: the loop logs it and
//     continues.
...
// where the two copies could drift (design risk #3).
//
...
var ErrSteer = errors.New("loopctl: steered")

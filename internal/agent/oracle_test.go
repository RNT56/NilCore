...
func (f fakeOracle) Plan(_ context.Context, _ string, candidates []string) agent.RoutePlan {
	...
	return agent.RoutePlan{Candidates: out, RaceN: f.raceN, EscalateAfter: f.escalateAfter}
}
...
func (f fakeOracle) EscalateAfter(_ string, def int) int {
	if f.escalateAfter > 0 {
		return f.escalateAfter
	}
	return def
}
...
func (emptyPlanOracle) EscalateAfter(_ string, def int) int { return def }
...
func TestPlanRoute_NilOracleByteIdentical(t *testing.T) {
	...
	if plan.RaceN != 0 || plan.EscalateAfter != 0 {
		t.Fatalf("nil oracle expressed sizing hints: raceN=%d escalateAfter=%d", plan.RaceN, plan.EscalateAfter)
	}
}
...
func TestOracleSizing_NilDefaultsUnchanged(t *testing.T) {
	...
	if got := agent.OracleEscalateAfter(nil, "refactor", 2); got != 2 {
		t.Fatalf("OracleEscalateAfter(nil) = %d, want default 2", got)
	}
}
...
func TestPlanRoute_FakeReordersAndSizes(t *testing.T) {
	cands := []string{"native", "codex", "claude-code"}
	o := fakeOracle{raceN: 4, escalateAfter: 1}
	...
	if plan.RaceN != 4 || plan.EscalateAfter != 1 {
		t.Fatalf("sizing not carried: raceN=%d escalateAfter=%d", plan.RaceN, plan.EscalateAfter)
	}
...
func TestOracleSizing_FakeOverrides(t *testing.T) {
	...
	// escalateAfter unset on the fake ⇒ default flows through.
	if got := agent.OracleEscalateAfter(o, "bugfix", 7); got != 7 {
		t.Fatalf("OracleEscalateAfter = %d, want default 7", got)
	}
}
